...
class Stack{
    ...
    errCode(0)  {        
        ...
        if( begPointer == nullptr ) errCode = BADPTR;
        else{
            errCode = 0;
...
void Stack::checkStack(){
    if( (this -> begPointer == (void*) -1) || (this -> begPointer == NULL) ) (this -> errCode) = BADPTR;
    else if( (this -> currentSize < 0) || (this -> currentSize > this -> maxSize) ) (this -> errCode) =  BADCURSZ;
    else if( this -> maxSize <= 0 ) (this -> errCode) = BADMAXSZ;
    else (this -> errCode) = 0;
}
